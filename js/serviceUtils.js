		...
		for (let i = 0; i < objParam.length; i++) {
			...
			switch (upper_menu_id) {
				case '1':
					...
					let main_bars_hamburger = document.querySelector('#main_bars_hamburger');

					strInnerHTML = setATagForMenu(objParam[i], '<h5>', '</h5>');

					main_bars.innerHTML +=
						"<ul class='multi-column-dropdown' id='" + menu_name + "_dropdown'>" + strInnerHTML + '</ul>';
					main_bars_hamburger.innerHTML +=
						"<ul class='multi-column-dropdown' id='" + menu_name + "_hamburger'>" + strInnerHTML + '</ul>';
					break;

				case '2':
					let restaurants_dropdown = document.querySelector('#restaurants_dropdown');
					let restaurants_hamburger = document.querySelector('#restaurants_hamburger');
				...
				case '24':
					...
					let main_beauty_hamburger = document.querySelector('#main_beauty_hamburger');
				...
				case '38':
					let entertainment_dropdown = document.querySelector('#entertainment_dropdown');
					let entertainment_hamburger = document.querySelector('#entertainment_hamburger');
   ...
   */
	...
	const setATagForMenu = (objMenuData, openTag, closeTag) => {
		...
		if (objMenuData.menu_page_yn === '1') {
			let movePageUrl = '../php/menu_detail.php';
   ...
   */
	const loadBiz = objParam => {
		...
		objParam.objBizData.map(bizData => {
          ...
          <div class="thumb-box">
            <div class="thumb-img">
                <a href="mamalee_level_${bizData.biz_level}.php?biz_id=${bizData.biz_id}">
                <img src=../${bizData.file_path.replace('/var/www/html/', 'public/')} width="100%" alt="" />
              </a>
            </div>
