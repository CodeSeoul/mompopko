...
(function prefillBuinessForm() {
  if (localStorage.getItem("selectedBusiness") != null) {
    ...
    document.querySelector('input[name="biz_id"]').value =
      business["biz_id"] || "";
    document.querySelector('input[name="file_grp_id"]').value =
      business["file_grp_id"] || "";
  }
})();
...
function createImageHandler(imagesObj) {
  ...
  for (index in imagesObj) {
    ...
    document.querySelector("form").insertBefore(container,document.querySelector('.buttons'));
  }
}
...
let addEventToForm = function () {
  let form = document.querySelector("#business_edit_form");
  console.log(form);
...
let createPreviewElement = function () {
  ...
  let operatingHour = form.querySelector('[name="openingHour"]').value;
