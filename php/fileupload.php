...
session_start();
    ...
    function fileupload_create_new_business(){
        try
        {           
    ...
    function fileupload_edit_business(){
        try
        {           
            $host = 'mompopkoapi.wcoding.com';
            $user = 'mompopko_admin';
        ...
        $fileGrpId = $_POST['file_grp_id'];
        $biz_menuName = $_POST['biz_menuName'];
        $newImages = $_FILES['newSubImages'];

        $replacingImages = [];
        foreach($_FILES as $key=>$value){
            if(preg_match("/newImage/",$key)){
                $replacingImages[$key] = $value;
            }
        }

        print_r($replacingImages);
        
        
        $imgDir = "../img/$biz_menuName/";

        //Replace Images
    
        foreach($replacingImages as $img=>$imgInfo){
            $file_id = (int)str_replace('newImage','',$img);
            echo "<br/><br/><br/><br/><br/><br/>";
            echo "This is file Id";
            echo $img;
            
            $currentImgPath = $pdo->query("SELECT * FROM tb_file WHERE file_grp_id = $fileGrpId AND file_id = $file_id")->fetch();

            print_r($currentImgPath);
        }
    }
